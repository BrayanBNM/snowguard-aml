...
ALERT_CASES = {
    "ACC_1042": {
        ...
        "structuring_query": """-- Cortex Semantic Verification: Pattern Structuring/Smurfing
...
WHERE account_id = 'ACC_1042'
  ...
  AND amount BETWEEN 9000.00 AND 10000.00;""",
        ...
        "audit_txns": [
            {"id": "TXN_50000", "time": "2026-09-01 10:00:00", "type": "DEPOSIT", "method": "CASH", "amount": 9450.00, "cp": "N/A", "country": "US"},
            {"id": "TXN_50001", "time": "2026-09-02 00:15:00", "type": "DEPOSIT", "method": "CASH", "amount": 9800.00, "cp": "N/A", "country": "US"},
            {"id": "TXN_50002", "time": "2026-09-02 14:40:00", "type": "DEPOSIT", "method": "CASH", "amount": 9120.00, "cp": "N/A", "country": "US"},
            {"id": "TXN_50003", "time": "2026-09-03 04:05:00", "type": "DEPOSIT", "method": "CASH", "amount": 9650.00, "cp": "N/A", "country": "US"},
            {"id": "TXN_50004", "time": "2026-09-03 06:30:00", "type": "TRANSFER_OUT", "method": "WIRE", "amount": 37500.00, "cp": "SHELL_CORP_KY_8829", "country": "KY"},
        ],
    ...
    "ACC_1085": {
        ...
        "structuring_query": """-- Cortex Semantic Verification: Business Structuring
SELECT * FROM TRANSACTIONS
WHERE account_id = 'ACC_1085'
  AND amount BETWEEN 9000.00 AND 10000.00;""",
        ...
        "audit_txns": [
            {"id": "TXN_50005", "time": "2026-09-03 09:00:00", "type": "DEPOSIT", "method": "ACH", "amount": 9950.00, "cp": "N/A", "country": "US"},
            {"id": "TXN_50006", "time": "2026-09-04 09:00:00", "type": "DEPOSIT", "method": "ACH", "amount": 9920.00, "cp": "N/A", "country": "US"},
            {"id": "TXN_50007", "time": "2026-09-05 09:00:00", "type": "DEPOSIT", "method": "ACH", "amount": 9980.00, "cp": "N/A", "country": "US"},
            {"id": "TXN_50008", "time": "2026-09-06 09:00:00", "type": "DEPOSIT", "method": "ACH", "amount": 9910.00, "cp": "N/A", "country": "US"},
            {"id": "TXN_50009", "time": "2026-09-06 13:00:00", "type": "TRANSFER_OUT", "method": "WIRE", "amount": 39500.00, "cp": "PANAMA_GLOBAL_CORP", "country": "PA"},
        ],
    ...
    "ACC_1102": {
        ...
        "structuring_query": """-- Cortex Semantic Verification: Velocity Smurfing
SELECT * FROM TRANSACTIONS
WHERE account_id = 'ACC_1102'
  AND transaction_type = 'DEPOSIT';""",
        ...
        "audit_txns": [
            {"id": "TXN_50010", "time": "2026-09-07 10:00:00", "type": "DEPOSIT", "method": "CASH", "amount": 8500.00, "cp": "N/A", "country": "US"},
            {"id": "TXN_50011", "time": "2026-09-07 12:30:00", "type": "DEPOSIT", "method": "CASH", "amount": 8900.00, "cp": "N/A", "country": "US"},
            {"id": "TXN_50012", "time": "2026-09-07 14:00:00", "type": "DEPOSIT", "method": "CASH", "amount": 9100.00, "cp": "N/A", "country": "US"},
        ],
...
with st.sidebar:
    ...
    selected_label = st.selectbox(
        "Select Flagged Case for Investigation:",
        options=case_labels,
        index=0
    )
    
    # Extract selected account ID
...
# Session state initialization for chat per account
...
# --- MAIN VIEW: FULL-WIDTH WORKSPACE TABS ---
...
with tab_investigation:
    ...
    # Sub-tabs for Chat vs Raw Data Table
    ...
    with chat_view:
        ...
        with c1:
            if st.button("🔍 Run Structuring Audit", use_container_width=True):
                ...
                <div style="background-color: #161b22; border-left: 3px solid #58a6ff; padding: 12px; border-radius: 4px; margin-bottom: 12px;">
                    ...
                    <small>Verified Semantic Query Executed:</small>
        ...
        with c2:
            if st.button("📜 Check FinCEN Policy", use_container_width=True):
                ...
                <div style="background-color: #161b22; border-left: 3px solid #bc8cff; padding: 12px; border-radius: 4px; margin-bottom: 12px;">
                    ...
                    <small>Source: <code>fincen_aml_guidelines.md</code> (Stage Path: <code>@AML_STAGE/fincen_aml_guidelines.md</code>)</small>
                </div>
        ...
        with c3:
            if st.button("📄 Generate SAR Summary", use_container_width=True):
        ...
        if user_query := st.chat_input("Ask Cortex Copilot about transactions, risk, or statutory requirements..."):
            ...
            st.session_state.messages.append({
                "role": "assistant", 
                "content": f"Analyzed query for `{selected_account}`: Criteria evaluated against Cortex Analyst semantic rules and regulatory guidance."
            })
            st.rerun()

    with ledger_view:
        st.markdown(f"#### Structured Transactions: `{selected_account}`")
        df_ledger = pd.DataFrame(case["audit_txns"])
...
with tab_sar:
    ...
    # Generate Dossier Markdown Text
    table_rows = "\n".join([
        f"| {t['id']} | {t['time']} | {t['type']} | {t['method']} | ${t['amount']:,.2f} | {t['cp']} | {t['country']} |"
...
"""
    ...
    with b_col1:
        st.button("🚀 Approve & File Regulatory SAR", type="primary", use_container_width=True)
    with b_col2:
        st.download_button(
